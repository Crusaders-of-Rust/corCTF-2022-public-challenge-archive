...
impl Context {
    ...
    pub fn get_reg(&self, idx: usize, k: usize) -> Option<usize> {
        if k - self.outer >= self.t_size {
            return None;
        }
        ...
        let parts = &self.memory[m][idx..idx+16];

        if parts.iter().all(|f| f.is_set) {
            return Some(parts.iter()
                .rfold(0, |acc,x| (acc << 1) | (x.val as usize)))
        }
